...
// Streaming message component that polls for updates
function StreamingMessage({
  ...
  useEffect(() => {
    if (isDriven && isGenerating) {
      fetch(`${convexUrl}/ask-ai-stream`, {
        method: "POST",
  ...
  }, [isDriven, convexUrl, streamId, isGenerating]);

  // Track if text is still being generated
  useEffect(() => {
    if (text.length > previousTextLength) {
      setPreviousTextLength(text.length);
      setIsGenerating(true);
    } else if (text.length > 0 && text.length === previousTextLength) {
      // Text stopped growing, mark as complete
      const timer = setTimeout(() => setIsGenerating(false), 2000);
      return () => clearTimeout(timer);
    }
  }, [text, previousTextLength]);

  const isLoading = isGenerating && text.length === 0;
  const showCopyButton = !isLoading && !!text;
  ...
  return (
    <div className="ask-ai-message ask-ai-message-assistant">
      <div className="ask-ai-message-content">
          ...
          <ReactMarkdown
            ...
            components={{
              a: ({ href, children }) => {
                ...
                if (href?.startsWith("/")) {
                  return (
                    <Link href={href} className="ask-ai-link">
                      {children}
                    </Link>
                  );
                }
        ...
        {isGenerating && text && <span className="ask-ai-cursor">|</span>}
      </div>
      {showCopyButton && (
        <button
...
export default function AskAIModal({ isOpen, onClose }: AskAIModalProps) {
  ...
  const convexCloudUrl = process.env.NEXT_PUBLIC_CONVEX_URL as string;
