...
// NOTE: Uses .take() with limit to avoid byte read limit (16MB)
export const listAll = query({
  ...
  handler: async (ctx) => {
    // Use .take() to limit results and avoid byte read limit
...
// Get pages with docs layout (for sidebar navigation)
export const getDocsPages = query({
  ...
  handler: async (ctx) => {
    ...
    // Filter pages with docs layout
    const docsPages = pages
      .filter((p) => p.layout === "sidebar")
      .sort((a, b) => {
        const orderA = a.order ?? 999;
        const orderB = b.order ?? 999;
        return orderA - orderB;
...
// Authenticated mutation for syncing pages from markdown files
