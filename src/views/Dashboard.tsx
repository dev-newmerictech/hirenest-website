...
import { Toast, ToastType, ToastContainer } from "@/app/components/dashboard/dashboard-toast";
import { CommandModal } from "@/app/components/dashboard/dashboard-command-modal";
import { ConfirmationModal } from "@/app/components/dashboard/dashboard-confirmation-modal";
import { SyncWarningModal } from "@/app/components/dashboard/dashboard-sync-warning-modal";
import { LoadingState, DashboardDisabled } from "@/app/components/dashboard/dashboard-loading-state";
...
// Main Dashboard export - auth is handled by middleware
...
// Dashboard content — accepts initialSection for sub-route support
export function DashboardContent({ initialSection = "posts" }: { initialSection?: DashboardSection }) {
  const { theme, setTheme } = useTheme();
  const { fontFamily, setFontFamily } = useFont();
  // const { user, signOut: supabaseSignOut } = useSupabaseAuth(); // Removed Supabase
  ...
  useEffect(() => {
    ...
    // Backward compatibility: If stored value is "true", we assume first user (or just log them out if unsafe) -> let's map to first user for dev convenience if needed, OR just require re-login.
    // Better: check if storedAuth is one of the valid emails.
    const user = USERS.find((u) => u.email === storedAuth);

    if (user) {
      setIsAuthenticated(true);
      setCurrentUserEmail(user.email);
    } else if (storedAuth === "true") {
      // Legacy "true" value -> map to default user or clear
      // Let's clear it to force re-login with email tracking
  ...
  // Server-side pagination for posts
  const [postsOffset, setPostsOffset] = useState(0);
  const POSTS_PER_PAGE = 15;

  // Convex queries
  const totalPostsCount = useQuery(api.posts.getTotalCount);
  const searchResults = useQuery(api.posts.searchDashboardPosts, searchQuery ? { query: searchQuery } : "skip");
  const paginatedPosts = useQuery(api.posts.listAll, { offset: postsOffset, limit: POSTS_PER_PAGE });
  const posts = searchQuery ? searchResults : paginatedPosts;
  const pages = useQuery(api.pages.listAll);
  const fullPost = useQuery(
    api.posts.getPostById,
    editingPostId ? { id: editingPostId } : "skip"
  );
  ...
  }>({ isOpen: false, postId: null, postTitle: "", item: null });
  ...
  const handleDeleteClick = useCallback((post: ContentItem) => {
    setDeleteModal({
      isOpen: true,
  ...
  const confirmDelete = useCallback(async () => {
    ...
    setIsDeleting(true);
    try {
      await deletePostMutation({ id: deleteModal.postId });
      addToast("Post deleted successfully", "success");
      setDeleteModal({ isOpen: false, postId: null, postTitle: "", item: null });
    } catch (error) {
  ...
  }, [deleteModal.postId, deletePostMutation, addToast]);
  ...
  const filteredPages = useMemo(() => {
    if (!pages) return undefined; // Return undefined to indicate loading state
  ...
  const handleEditPost = useCallback((post: ContentItem) => {
    // If content is missing, set the post ID to fetch full post
    if (!post.content) {
      setEditingPostId(post._id as Id<"posts">);
    } else {
  ...
  // Update editing item when full post is loaded
  useEffect(() => {
    if (fullPost && editingPostId && !editingItem?.content) {
      const post = posts?.find((p) => p._id === editingPostId);
      if (post) {
        setEditingItem({ ...post, content: fullPost.content });
        setEditingPostId(null);
      }
    }
  }, [fullPost, editingPostId, editingItem, posts]);
  ...
  const doSavePost = useCallback(
    async (shouldPublish = false) => {
      ...
      const fieldsToCheck = editingItem.order !== undefined ? PAGE_FIELDS : POST_FIELDS;
      const missingFields = fieldsToCheck
        .filter((field) => field.required)
        .filter((field) => {
          const value = editingItem[field.name as keyof ContentItem];
          // Check for empty array (tags) or empty string/null/undefined
      ...
      try {
        ...
        await updatePostMutation({
          ...
          published: shouldPublish ? true : item.published,
          tags: item.tags || [],
          readTime: item.readTime, // Assuming readTime is calculated or user input
          image: item.image,
          showImageAtTop: item.showImageAtTop, // Need to ensure type exists on ContentItem
    ...
    [editingItem, updatePostMutation, addToast],
  );
  ...
  const handleSavePost = useCallback(
    async (shouldPublish?: boolean) => {
      ...
      if (editingItem.source === "sync" || !editingItem.source) {
        // Show warning modal
        setSyncWarningModal({ isOpen: true, item: editingItem, type: "post" });
      } else {
        // Dashboard-created content, save directly
        await doSavePost(shouldPublish);
      }
    },
    [editingItem, doSavePost],
  );
  ...
  const handleCopyBeforeDelete = useCallback(async () => {
    ...
    const markdown = generateMarkdown(deleteModal.item, "post");
  ...
  // Font toggle
  const toggleFont = () => {
    const fonts: Array<"serif" | "sans" | "monospace" | "jakarta"> = [
      "serif",
      "sans",
      "monospace",
      "jakarta",
  ...
  void showAuthWarning; // Reserved for future use
              ...
              <VStack gap="5" align="stretch">
                {loginError && (
                  <Box bg="red.50" borderRadius="md" p="3" borderWidth="1px" borderColor="red.200">
                    <Text color="red.600" textAlign="center" fontSize="sm" fontWeight="medium">
                      {loginError}
                    </Text>
                  </Box>
                )}
                ...
                <Box>
                  <Text mb="2" fontWeight="600" color="#2d3748">Email Address</Text>
                  <Input
                    type="email"
                ...
                <Box>
                  <Text mb="2" fontWeight="600" color="#2d3748">Password</Text>
                  <Input
                    type="password"
    ...
    >
      ...
      {/* {showAuthWarning && (
        <div className="dashboard-auth-warning">
          ...
          <span>
            Dashboard access is open. Set requireAuth: true in siteConfig.ts for secure access.
          </span>
        </div>
      )} */}
      ...
      >
        <div className="dashboard-sidebar-header">
          <Link href="/blog" className="dashboard-logo-link" title="Back to home">
          ...
          {navSections.map((section) => (
            <div key={section.label} className="dashboard-nav-section">
              ...
              {section.items.map((item) => {
                // Redirect to /write page for Write Post and Write Page
                return (
                  <button
                    key={item.id}
        ...
        <div className="dashboard-sidebar-footer">
          <div className="dashboard-user-card">
            <div className="dashboard-user-avatar" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#e2e8f0' }}>
...
function PostsListView({
  ...
  onDelete: (post: ContentItem) => void;
  searchQuery: string;
  totalPostsCount: number;
  postsOffset: number;
  setPostsOffset: (offset: number) => void;
  postsPerPage: number;
}) {
  const [filter, setFilter] = useState<"all" | "published" | "draft">("all");

  // Apply client-side filter (published/draft status + search on current batch)
  const filteredPosts = useMemo(() => {
    ...
    // Search filtering is now handled by backend query
    // if (searchQuery.trim()) { ... }
  ...
  const currentPage = Math.floor(postsOffset / postsPerPage);
  const totalPages = Math.ceil(totalPostsCount / postsPerPage);
  const hasNextPage = currentPage < totalPages - 1;
  const hasPrevPage = currentPage > 0;

  // Pagination handlers - fetch new batch from server
  ...
  const handlePrevPage = () => {
    if (hasPrevPage) {
      setPostsOffset(Math.max(0, postsOffset - postsPerPage));
    }
  };

  const handleNextPage = () => {
    if (hasNextPage) {
      setPostsOffset(postsOffset + postsPerPage);
    }
  };

  const handleFilterChange = (newFilter: "all" | "published" | "draft") => {
    setFilter(newFilter);
    setPostsOffset(0); // Reset to first page when filter changes
  };
      ...
      <div className="dashboard-list-table">
        ...
        {filteredPosts === undefined ? (
          <div className="dashboard-list-loading">
            <SpinnerGap className="dashboard-loading-spinner" size={24} />
...
function PagesListView({
  ...
  const filteredPages = useMemo(() => {
    // Show loading state if pages is undefined
  ...
  const paginatedPages = useMemo(() => {
    if (!filteredPages) return undefined;
    const start = currentPage * itemsPerPage;
    return filteredPages.slice(start, start + itemsPerPage);
  }, [filteredPages, currentPage, itemsPerPage]);

  const totalPages = filteredPages ? Math.ceil(filteredPages.length / itemsPerPage) : 0;
  const hasNextPage = filteredPages ? currentPage < totalPages - 1 : false;
      ...
      <div className="dashboard-list-header">
        <div className="dashboard-filter-tabs">
          ...
          <button
            ...
            disabled={!pages}
          >
            Drafts ({pages?.filter((p) => !p.published).length ?? 0})
          </button>
...
function EditorView({
  ...
  void versionControlEnabled; // Used in JSX
          ...
          {type === "post" && (
            <>
              {!item.published && (
                ...
                >
                  {isSaving ? (
                    <SpinnerGap className="spinning" size={16} />
                  ) : (
              ...
              >
                ...
                <span>{item.published ? "Update and Publish" : "Save Draft"}</span>
              </button>
            </>
          )}
...
function WriteSection({ contentType, setActiveSection: _setActiveSection }: { contentType: "post" | "page"; setActiveSection: (section: DashboardSection) => void; }) {
  const [content, setContent] = useState("");
  const [copied, setCopied] = useState(false);
  const [copiedField, setCopiedField] = useState<string | null>(null);
  const [editorMode] = useState<"markdown" | "richtext" | "preview">("markdown");
  const [isSaving, setIsSaving] = useState(false);
  ...
  // Save to database (Publish to DP)
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (frontmatter.published) {
          ...
          window.open(`/blog/${frontmatter.slug}`, '_blank', 'noopener,noreferrer');
        }
      }, 2000);
    } catch (error) {
  ...
  }, [content, createOrUpdatePost, router2]);
  ...
  return (
    <div className="dashboard-write-section">
      ...
      <div className="dashboard-write-header">
        ...
        <div className="dashboard-write-actions">
          ...
          <button
            ...
            className={`dashboard-action-btn primary ${saveSuccess ? "success" : ""}`}
            disabled={isSaving}
      ...
      {/* Save error message */}
      {saveError && (
        <div className="dashboard-write-error" style={{
          padding: "12px 16px",
          marginTop: "12px",
...
function IndexHtmlSection({
  ...
  const [htmlConfig, setHtmlConfig] = useState({
    ...
    siteDomain: process.env.NEXT_PUBLIC_SITE_URL
  ...
  return (
    <div className="dashboard-stats-section">
      ...
      <div className="dashboard-stats-link">
        <Link href="/stats" className="dashboard-placeholder-link">
...
function SyncSection({
  ...
  syncOutputRef: React.RefObject<HTMLPreElement | null>;
