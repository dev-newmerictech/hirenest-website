...
// Generate upload URL using Convex's built-in storage (handles CORS properly)
...
// Commit file uploaded via Convex storage
// This is the CORS-friendly upload flow when Bunny CDN is not configured:
// 1. Client calls generateUploadUrl() → gets signed URL
// 2. Client uploads to signed URL → gets storageId
// 3. Client calls commitConvexUpload() with storageId → stores file info
export const commitConvexUpload = action({
  args: {
...
// Get URL for a Convex storage file
