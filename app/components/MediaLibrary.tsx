...
// Error boundary to catch Convex connection errors
class MediaLibraryErrorBoundary extends Component<
  { children: ReactNode },
...
> {
  ...
  render() {
    if (this.state.hasError) {
      ...
      return (
        <div className="media-library-error" style={{
          display: "flex",
          ...
          <Terminal size={48} weight="duotone" style={{ marginBottom: "1rem", opacity: 0.6 }} />
          <h3 style={{ margin: "0 0 0.5rem", color: "var(--text-primary)" }}>Media Library Unavailable</h3>
          {isConvexError ? (
            <>
              <p style={{ margin: "0 0 1rem" }}>Convex backend functions are not deployed.</p>
              <code style={{
...
function MediaLibraryContent() {
  ...
  const commitConvexUpload = useAction(api.files.commitConvexUpload);
  ...
  // Track Convex storage uploads (when Bunny CDN is not configured)
  const [convexUploads, setConvexUploads] = useState<Map<string, { storageId: Id<"_storage">; url: string }>>(new Map());
