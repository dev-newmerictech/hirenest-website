...
// Generate embedding for text using OpenRouter or OpenAI
export const generateEmbedding = internalAction({
  ...
  handler: async (_ctx, { text }) => {
    ...
    if (!apiKey) {
      throw new Error("OPENROUTER_API_KEY or OPENAI_API_KEY not configured in Convex environment");
    }
...
export const generateMissingEmbeddings = action({
  ...
  handler: async (ctx): Promise<{
    ...
    if (!process.env.OPENROUTER_API_KEY && !process.env.OPENAI_API_KEY) {
      console.log("OPENROUTER_API_KEY or OPENAI_API_KEY not set, skipping embedding generation");
